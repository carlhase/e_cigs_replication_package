...
outpath = "D:/convenience_store/data/processed/LS_Otter/indexes/vape_qty_per_trans_indexes_1a/"
...
for store in store_numbers:
    ...
    monthly_qty_count = (
        ...
        .agg(qty_count = ('quantity', 'sum'),
             trans_count = ('transaction_count', 'sum'))
        .reset_index()
        )
    
    
    # qty per transaction
    monthly_qty_count['qty_per_trans'] = monthly_qty_count['qty_count']/monthly_qty_count['trans_count']
    ...
    monthly_qty_count['lag_qty_per_trans'] = monthly_qty_count.groupby(['store_id'])['qty_per_trans'].shift(1)
    
    # Set lagged values to NaN if the time difference is not exactly 1 month
    monthly_qty_count['lag_qty_per_trans'] = np.where(monthly_qty_count['month_diff'] == 1, monthly_qty_count['lag_qty_per_trans'], np.nan)
    
    monthly_qty_count = monthly_qty_count.fillna(value=np.nan)
    
    monthly_qty_count['qty_per_trans_index'] = (monthly_qty_count['qty_per_trans']/monthly_qty_count['lag_qty_per_trans'])
    
    monthly_qty_count['l_qty_per_trans_index'] = np.log(monthly_qty_count['qty_per_trans_index'])
...
# save indexes
indexes.to_feather('D:/convenience_store/data/processed/LS_Otter/indexes/vape_qty_per_trans_index_1a.feather')
