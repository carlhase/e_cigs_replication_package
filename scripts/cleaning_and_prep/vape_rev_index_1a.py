...
outpath = "D:/convenience_store/data/processed/LS_Otter/indexes/vape_rev_indexes_1a/"
...
for store in store_numbers:
    ...
    #subcat_df.rename(columns = {'qtyaction_count': 'rev'}, inplace = True)

    # monthly total
    monthly_rev = (
        subcat_df
        .groupby(['store_id', 'date'])
        .agg(rev = ('total_revenue_amount', 'sum'))
        .reset_index()
        )
    
    # Sort the DataFrame by store_id, product_id, and date
    monthly_rev = monthly_rev.sort_values(by=['store_id', 'date'])
    
    # Calculate the month difference between consecutive rows within each group
    monthly_rev['month_diff'] = monthly_rev.groupby(['store_id'])['date'].diff().apply(lambda x: x.n if pd.notna(x) else np.nan)
    
    # Group by 'store_id' and 'product_id' and calculate the lagged 'unit_value' only for consecutive months
    monthly_rev['lag_rev'] = monthly_rev.groupby(['store_id'])['rev'].shift(1)
    
    # Set lagged values to NaN if the time difference is not exactly 1 month
    monthly_rev['lag_rev'] = np.where(monthly_rev['month_diff'] == 1, monthly_rev['lag_rev'], np.nan)
    
    monthly_rev = monthly_rev.fillna(value=np.nan)

    
    monthly_rev['rev_index'] = (monthly_rev['rev']/monthly_rev['lag_rev'])
    
    monthly_rev['l_rev_index'] = np.log(monthly_rev['rev_index'])
    ...
    pa.feather.write_feather(monthly_rev, output_filename)
...
# list of files (stores)
source_dir = "D:/convenience_store/data/processed/LS_Otter/indexes/vape_rev_indexes_1a/"
...
# save indexes
indexes.to_feather('D:/convenience_store/data/processed/LS_Otter/indexes/vape_rev_index_1a.feather')
