...
outpath = "D:/convenience_store/data/processed/LS_Otter/indexes/vape_qty_index_1d/"
...
for store in store_numbers:
    ...
    brand_revenue = (
        subcat_df
        .groupby(['store_id', 'subcategory', 'brand', 'fiscal_year'], dropna = False)
        .agg(brand_annual_revenue = ('total_revenue_amount', 'sum'))
        .reset_index()
        )
    ...
    product_revenue = (
        subcat_df
        .groupby(['store_id', 'subcategory', 'brand', 'gtin', 'fiscal_year'], dropna = False)
        .agg(product_annual_revenue = ('total_revenue_amount', 'sum'))
        .reset_index()
        )

    # stage 1 weight: product share of category revenue (first-stage weight)
    stage_1_weight = pd.merge(product_revenue, brand_revenue,
                              how = 'left',
                              on = ['store_id', 'subcategory', 'brand', 'fiscal_year']
                              )

    stage_1_weight['stage_1_weight'] = stage_1_weight['product_annual_revenue']/stage_1_weight['brand_annual_revenue']

    # merge stage 1 weight
    stage_1_df = pd.merge(subcat_df, stage_1_weight,
                          how = 'left',
                          on = ['store_id', 'subcategory', 'brand', 'gtin', 'fiscal_year']
                          )
    ...
    brand_index = (
        stage_1_df
        .groupby(['store_id', 'subcategory', 'brand', 'date'], dropna = False)
        .agg(brand_index = ('unit_qty_index', 'prod'))
        .reset_index()
        )
    
    # stage 2 weight: type share of (sub)category
    stage_2_weight = pd.merge(brand_revenue, category_revenue,
                              how = 'left',
                              on = ['store_id', 'subcategory', 'fiscal_year']
                              )

    stage_2_weight['stage_2_weight'] = stage_2_weight['brand_annual_revenue']/stage_2_weight['category_annual_revenue']
    ...
    brand_index = pd.merge(brand_index, fisc_year_date_df,
                           how = 'left',
                           on = ['date']
                           )
    ...
    stage_2_df = pd.merge(brand_index, stage_2_weight,
                          how = 'left',
                          on = ['store_id', 'subcategory', 'brand', 'fiscal_year']
                          )

    stage_2_df['weighted_brand_index'] = stage_2_df['brand_index']**stage_2_df['stage_2_weight']
    
    # aggregate to type index (stage 1)
    store_index = (
        stage_2_df
        .groupby(['store_id', 'subcategory', 'date'])
        .agg(vape_qty_index_1d = ('weighted_brand_index', 'prod'))
        .reset_index()
    ...
    store_index['l_vape_qty_index_1d'] = np.log(store_index['vape_qty_index_1d'])
...
source_dir = "D:/convenience_store/data/processed/LS_Otter/indexes/vape_qty_index_1d/"
...
# save indexes
indexes.to_feather('D:/convenience_store/data/processed/LS_Otter/indexes/vape_qty_index_1d.feather')
